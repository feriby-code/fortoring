        ...
        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
            ...
            if (isset($_POST['btn_filter_coll_std'])) {
                ...
                if ($activity_coll_std != "NULL" && $filter_coll_std == "NULL" && $information_coll_std == "NULL") {
                    $precense_coll_std = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT($activity_coll_std) AS precense FROM college_student_presence WHERE $activity_coll_std='1';"));

                    $notPrecense_coll_std = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT($activity_coll_std) AS not_precense FROM college_student_presence WHERE $activity_coll_std='0';"));

                    $labels_temp_coll_std = ['Hadir', 'Tidak Hadir'];
                    $datas_temp_coll_std = [$precense_coll_std['precense'], $notPrecense_coll_std['not_precense']];
        ...
        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 d-flex flex-column align-items-center">
            ...
            if (isset($_POST['btn_filter_mgt'])) {
                ...
                if ($activity_mgt != "NULL" && $filter_mgt == "NULL" && $information_mgt == "NULL") {
                    $precense_mgt = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT($activity_mgt) AS precense FROM management_presence WHERE $activity_mgt='1';"));

                    $notPrecense_mgt = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT($activity_mgt) AS not_precense FROM management_presence WHERE $activity_mgt='0';"));

                    $labels_temp_mgt = ['Hadir', 'Tidak Hadir'];
                    $datas_temp_mgt = [$precense_mgt['precense'], $notPrecense_mgt['not_precense']];
